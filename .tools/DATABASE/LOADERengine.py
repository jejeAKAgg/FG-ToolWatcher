...
class LoaderEngine:
    ...
    def _extract_field(self, soup: BeautifulSoup, field: str, jsonld: dict) -> str:
        ...
        if self.WEBSITEcfg.get("jsonld") is True:
            ...
            # if DATA from JSON-LD, validate before returning
            if result:
                result_str = str(result).strip()
                PLACEHOLDERS = {"-", "", "NAN", "NONE", "NULL"}

                # MPN specific — reject if identical to EAN (JSON-LD often sets mpn=sku=ean)
                if field == "mpn":
                    ean_result = None
        ...
        if isinstance(sel, dict):
            ...
            # ── Attribut custom / tag simple (Toolnation/Klium price) ──
                ...
                find_kwargs = {k: v for k, v in [('class_', cls), ('id', id_)] if v}
                # If split_on defined, find all matching elements and pick the one containing the keyword
                if split_on:
            ...
            if result and split_on:
                ...
                if len(parts) > 1:
                    # Take first word after split, strip pipe and special chars
