...
class LECOTwatcher:
    ...
    def _load_db(self) -> Optional[pd.DataFrame]:
        ...
        if not os.path.exists(master_path):
            LOG.warning(f"[LECOTwatcher] MASTER_DB introuvable : {master_path}")
            return None
        ...
        try:
            ...
            LOG.debug(f"[LECOTwatcher] DB chargée — {len(df)} produits.")
            return df.reset_index(drop=True)
        except Exception as e:
            LOG.exception(f"[LECOTwatcher] Erreur chargement MASTER_DB : {e}")
            return None
        ...
        while self.ATTEMPT < self.MAX_RETRIES:
            try:
                ...
                if response.status_code == 404:
                    LOG.warning(f"[LECOTwatcher] 404 pour {link}")
                    return PRODUCTvar
                LOG.exception(f"[LECOTwatcher] HTTP {response.status_code} : {http_err}")
                self.ATTEMPT += 1
                time.sleep(self.RETRY_DELAY)

            except Exception as e:
                LOG.exception(f"[LECOTwatcher] Erreur extraction {link} : {e}")
                self.ATTEMPT += 1
                time.sleep(self.RETRY_DELAY)

        LOG.warning(f"[LECOTwatcher] Abandon après {self.MAX_RETRIES} tentatives pour {link}")
        return None
    ...
    def run(self) -> pd.DataFrame:
        ...
        CSVpath  = os.path.join(RESULTS_SUBFOLDER_TEMP, "LECOTproducts.csv")
        XLSXpath = os.path.join(RESULTS_SUBFOLDER_TEMP, "LECOTproducts.xlsx")
        ...
        try:
            for item in self.items:
                ...
                if CACHED := self._cache_checker(cache_df=CACHEdata, item=item_name):
                    LOG.debug(f"[LECOTwatcher] Cache hit : {item_name}")
                ...
                if db_row:
                    ...
                    LOG.warning(f"[LECOTwatcher] Produit absent de la DB Fixami — {item_name} (EAN={item.get('ean')} / MPN={item.get('mpn')})")

                time.sleep(random.uniform(1.5, 3))

        except Exception as e:
            LOG.error(f"[LECOTwatcher] Erreur fatale : {e}")

        df = pd.DataFrame(PRODUCTS)
        df.to_csv(CSVpath,  index=False, encoding='utf-8-sig')
        df.to_excel(XLSXpath, index=False)

        LOG.debug("[LECOTwatcher] Terminé.")
        return df