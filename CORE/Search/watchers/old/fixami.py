...
class FIXAMIwatcher:
    ...
    def _load_db(self) -> Optional[pd.DataFrame]:
        ...
        if not os.path.exists(master_path):
            LOG.warning(f"[FIXAMIwatcher] MASTER_DB introuvable : {master_path}")
            return None
        ...
        try:
            ...
            LOG.debug(f"[FIXAMIwatcher] DB chargée — {len(df)} produits.")
            return df.reset_index(drop=True)
        except Exception as e:
            LOG.exception(f"[FIXAMIwatcher] Erreur chargement MASTER_DB : {e}")
            return None
        ...
        while self.ATTEMPT < self.MAX_RETRIES:
            try:
                ...
                if response.status_code == 404:
                    LOG.warning(f"[FIXAMIwatcher] 404 pour {link}")
                    return PRODUCTvar
                LOG.exception(f"[FIXAMIwatcher] HTTP {response.status_code} : {http_err}")
                self.ATTEMPT += 1
                time.sleep(self.RETRY_DELAY)

            except Exception as e:
                LOG.exception(f"[FIXAMIwatcher] Erreur extraction {link} : {e}")
                self.ATTEMPT += 1
                time.sleep(self.RETRY_DELAY)

        LOG.warning(f"[FIXAMIwatcher] Abandon après {self.MAX_RETRIES} tentatives pour {link}")
        return None
        ...
        try:
            for item in self.items:
                ...
                if CACHED := self._cache_checker(cache_df=CACHEdata, item=item_name):
                    LOG.debug(f"[FIXAMIwatcher] Cache hit : {item_name}")
                ...
                if db_row:
                    ...
                    LOG.warning(f"[FIXAMIwatcher] Produit absent de la DB Fixami — {item_name} (EAN={item.get('ean')} / MPN={item.get('mpn')})")

                time.sleep(random.uniform(1.5, 3))

        except Exception as e:
            LOG.error(f"[FIXAMIwatcher] Erreur fatale : {e}")

        df = pd.DataFrame(PRODUCTS)
        df.to_csv(CSVpath,  index=False, encoding='utf-8-sig')
        df.to_excel(XLSXpath, index=False)

        LOG.debug("[FIXAMIwatcher] Terminé.")
        return df